...
export const vendasRouter = router({
  // ─── PEDIDOS ───────────────────────────────────────────────────────────────
  listPedidos: protectedProcedure.input(z.object({ empresaId: z.number().optional(), status: z.string().optional(), search: z.string().optional() })).query(async ({ input, ctx }) => {
    const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    const empresaId = await resolveAccessibleEmpresaId(ctx, input.empresaId);
    const conds: any[] = [eq(pedidos.empresaId, empresaId), isNull(pedidos.deletedAt)];
    ...
    try {
      return await db.select().from(pedidos).where(and(...conds)).orderBy(desc(pedidos.createdAt));
    } catch {
      const legacyConds: any[] = [eq(pedidos.empresaId, empresaId)];
      if (input.status && input.status !== "todos") legacyConds.push(eq(pedidos.status, input.status as any));
      if (input.search) legacyConds.push(or(ilike(pedidos.clienteNome, `%${input.search}%`), ilike(pedidos.numero, `%${input.search}%`))!);
      return db.select().from(pedidos).where(and(...legacyConds)).orderBy(desc(pedidos.createdAt));
    }
  }),
  createPedido: protectedProcedure.input(z.object({
    ...
    const { itens, empresaId: _empresaId, ...pedidoData } = input;
    const [p] = await db.insert(pedidos).values({ ...pedidoData, numero, valorTotal, empresaId, vendedorId: ctx.user.id, createdBy: ctx.user.id }).returning();
  ...
  updatePedidoStatus: protectedProcedure.input(z.object({
    empresaId: z.number().optional(), id: z.number(), status: z.enum(["rascunho", "enviado", "aprovado", "em_separacao", "expedido", "entregue", "cancelado"]),
  })).mutation(async ({ input, ctx }) => {
    const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    const empresaId = await resolveAccessibleEmpresaId(ctx, input.empresaId);
    await db.update(pedidos).set({ status: input.status, updatedAt: new Date() }).where(and(eq(pedidos.id, input.id), eq(pedidos.empresaId, empresaId)));
    return { success: true };
  }),
  ...
  // ─── PROPOSTAS ─────────────────────────────────────────────────────────────
  listPropostas: protectedProcedure.input(z.object({ empresaId: z.number().optional(), status: z.string().optional(), search: z.string().optional() })).query(async ({ input, ctx }) => {
    const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    const empresaId = await resolveAccessibleEmpresaId(ctx, input.empresaId);
    const conds: any[] = [eq(propostas.empresaId, empresaId), isNull(propostas.deletedAt)];
    ...
    try {
      return await db.select().from(propostas).where(and(...conds)).orderBy(desc(propostas.createdAt));
    } catch {
      const legacyConds: any[] = [eq(propostas.empresaId, empresaId)];
      if (input.status && input.status !== "todos") legacyConds.push(eq(propostas.status, input.status as any));
      if (input.search) legacyConds.push(ilike(propostas.titulo, `%${input.search}%`));
      return db.select().from(propostas).where(and(...legacyConds)).orderBy(desc(propostas.createdAt));
    }
  }),
  createProposta: protectedProcedure.input(z.object({
    ...
    const { empresaId: _empresaId, ...payload } = input;
    const [p] = await db.insert(propostas).values({ ...payload, numero, empresaId, vendedorId: ctx.user.id, createdBy: ctx.user.id }).returning();
    return p;
  }),
  ...
  // ─── DASHBOARD VENDAS ──────────────────────────────────────────────────────
  dashboard: protectedProcedure.input(z.object({ empresaId: z.number().optional() }).optional()).query(async ({ ctx, input }) => {
    const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    const empresaId = await resolveAccessibleEmpresaId(ctx, input?.empresaId);
    let pedidoStats: any;
    let propostaStats: any;
    try {
      [pedidoStats] = await db.select({
      ...
      try {
        [pedidoStats] = await db.select({
          ...
          valorTotal: sql<string>`coalesce(sum("valorTotal"::numeric), 0)::text`,
          entregues: sql<number>`count(*) filter (where status = 'entregue')`,
        }).from(pedidos).where(eq(pedidos.empresaId, empresaId));
      } catch {
        try {
          [pedidoStats] = await db.select({
            total: sql<number>`count(*)`,
          ...
          pedidoStats = { total: 0, valorTotal: "0", entregues: 0 };
        }
      }
    }
    try {
      [propostaStats] = await db.select({
        total: sql<number>`count(*)`,
      ...
      [propostaStats] = await db.select({
        total: sql<number>`count(*)`,
        aprovadas: sql<number>`count(*) filter (where status = 'aprovada')`,
      }).from(propostas).where(eq(propostas.empresaId, empresaId));
    }
