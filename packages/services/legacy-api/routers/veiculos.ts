...
const veiculoInput = z.object({
  ...
  capacidadeCarga: z.union([z.string(), z.number()]).nullable().optional(),
  motoristaId: z.number().nullable().optional(),
  ajudanteId: z.number().nullable().optional(),
  kmAtual: z.number().nullable().optional(),
  mediaConsumo: z.union([z.string(), z.number()]).nullable().optional(),
    ...
    .mutation(async ({ input, ctx }) => {
      return safeDb(async () => {
        ...
        const [result] = await db.insert(veiculos).values({
          ...input,
          empresaId,
          capacidadeCarga: input.capacidadeCarga ? String(input.capacidadeCarga).replace(',', '.') : null,
          mediaConsumo: input.mediaConsumo ? String(input.mediaConsumo).replace(',', '.') : null,
